...
{
	return sps != NULL && sps->pic_width_in_luma_samples != 0;
}
...
static inline void cpy_colour(struct v4l2_pix_format * d, const struct v4l2_pix_format * s)
{
				  ...
				  struct v4l2_format *f)
{
	...
	if ((rv = rpivid_prepare_src_format(&f->fmt.pix)) != 0)
	{
		// Set default src format
		f->fmt.pix.pixelformat = RPIVID_SRC_PIXELFORMAT_DEFAULT;
		rv = rpivid_prepare_src_format(&f->fmt.pix);
	}
	return rv;
}

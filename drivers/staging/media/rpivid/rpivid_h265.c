...
static int gptr_realloc_new(struct rpivid_dev * const dev, struct rpivid_gptr * const gptr, size_t size)
{
    if (size == gptr->size)
        return 0;
...
static void aux_q_release(struct rpivid_ctx * const ctx, rpivid_q_aux_t ** const paq)
{
...
#warning "EMU set - probably shouldn't be wanted"
    ...
    if (s->src_addr != 0) {
        ...
        memcpy(de->bit_copy_gptr->ptr + de->bit_copy_len, s->src_buf + offset, len);
...
// Simply checks POCs
static int has_backward(const struct v4l2_hevc_dpb_entry * const dpb, const __u8 * const idx, const unsigned int n, const unsigned int cur_poc)
{
    unsigned int i;
    for (i = 0; i < n; ++i) {
        // Compare mod 2^16
        ...
        //  of DiffPicOrderCnt( picA, picB ) used in the decoding process
        //  that are not in the range of −2^15 to 2^15 − 1, inclusive."
...
static int frame_end(struct rpivid_dev * const dev, dec_env_t * const de, const dec_state_t * const s)
{
    ...
    if (!de->cmd_copy_gptr->ptr || cmd_size > de->cmd_copy_gptr->size) {
        ...
        if (gptr_realloc_new(dev, de->cmd_copy_gptr, cmd_alloc)) {
            v4l2_err(&dev->v4l2_dev, "Alloc cmd buffer (%d): FAILED\n", cmd_alloc);
            return -ENOMEM;
        }
...
static void setup_colmv(struct rpivid_ctx * const ctx, struct rpivid_run *run, dec_state_t * const s)
{
...
//    v4l2_info(&dev->v4l2_dev, "vptr=%p\n", vb2_plane_vaddr(&run->src->vb2_buf, 0));
    ...
    if ((de = ctx->dec0) != NULL && de->state != RPIVID_DECODE_END) {
        ...
        if (!s->frame_end && !de->bit_copy_gptr->ptr) {
            ...
            // Annex A gives a min compression of 2 @ lvl 3.1 (wxh <= 983040)
            // and min 4 thereafter but avoid the odity of 983041 having a
            // lower limit than 983040. Multiply by 3/2 for 4:2:0
            ...
            {
                v4l2_err(&dev->v4l2_dev, "Unable to alloc buf (%d) for bit copy\n", bits_alloc);
                goto fail;
            }
            v4l2_info(&dev->v4l2_dev, "Alloc buf (%d) for bit copy OK\n", bits_alloc);
        }
	}

    // Pre calc a few things
    s->src_addr = !s->frame_end ? 0 : vb2_dma_contig_plane_dma_addr(&run->src->vb2_buf, 0);
    s->src_buf = s->src_addr != 0 ? NULL : vb2_plane_vaddr(&run->src->vb2_buf, 0);
...
static int check_status(const struct rpivid_dev * const dev) {
    ...
    uint32_t status = apb_read(dev, RPI_STATUS);
...
static void cb_phase2(struct rpivid_dev * const dev, void * v)
{
    ...
    v4l2_m2m_cap_buf_return(dev->m2m_dev, ctx->fh.m2m_ctx, de->frame_buf, VB2_BUF_STATE_DONE);
    de->frame_buf = NULL;
...
static void phase1_thread(struct rpivid_dev * const dev, void * v)
{
    ...
    if (de->p1_status & STATUS_PU_EXHAUSTED) {
        if (gptr_realloc_new(dev, pu_gptr, next_size(pu_gptr->size))) {
            v4l2_err(&dev->v4l2_dev, "%s: PU realloc (%#x) failed\n", __func__, pu_gptr->size);
            goto fail;
        }
    ...
    if (de->p1_status & STATUS_COEFF_EXHAUSTED) {
        if (gptr_realloc_new(dev, coeff_gptr, next_size(coeff_gptr->size))) {
            v4l2_err(&dev->v4l2_dev, "%s: Coeff realloc (%#x) failed\n", __func__, coeff_gptr->size);
            goto fail;
        }
...
static void cb_phase1(struct rpivid_dev * const dev, void * v)
{
    ...
    if (de->p1_status != 0)
    {
        v4l2_err(&dev->v4l2_dev, "%s: Post wait: %#x\n", __func__, de->p1_status);
    ...
    if (de->frame_buf == NULL)
    {
        v4l2_err(&dev->v4l2_dev, "%s: No detached buffer\n", __func__);
        goto fail;
    }

    ctx->p2idx = (ctx->p2idx + 1 >= RPIVID_P2BUF_COUNT) ? 0 : ctx->p2idx + 1;
...
static void phase1_claimed(struct rpivid_dev * const dev, void * v)
{
    ...
    const struct rpivid_gptr * const coeff_gptr = ctx->coeff_bufs + ctx->p2idx;
    ...
    de->coeff_stride = ALIGN_DOWN(coeff_gptr->size / de->PicHeightInCtbsY, 64);
...
static int rpivid_h265_start(struct rpivid_ctx *ctx)
{
    ...
    for (i = 0; i != ARRAY_SIZE(ctx->cmdbufs); ++i)
    {
        if (gptr_alloc(dev, ctx->cmdbufs + i,
                       0x4000,
                       DMA_ATTR_FORCE_CONTIGUOUS) != 0)
            goto fail;
    }
    ...
    for (i = 0; i != ARRAY_SIZE(ctx->pu_bufs); ++i)
    {
        ...
        if (gptr_alloc(dev, ctx->pu_bufs + i,
                       pu_alloc,
                       DMA_ATTR_FORCE_CONTIGUOUS | DMA_ATTR_NO_KERNEL_MAPPING) != 0)
            goto fail;
        if (gptr_alloc(dev, ctx->coeff_bufs + i,
                       coeff_alloc,
                       DMA_ATTR_FORCE_CONTIGUOUS | DMA_ATTR_NO_KERNEL_MAPPING) != 0)
            goto fail;
    }
...
//	v4l2_info(&dev->v4l2_dev, "rpivid_h265_trigger start\n");
    ...
    switch (de == NULL ? RPIVID_DECODE_ERROR_CONTINUE : de->state) {
        ...
        case RPIVID_DECODE_PHASE1:
            ctx->dec0 = NULL;
            rpivid_hw_irq_active1_claim(dev, &de->irq_ent, phase1_claimed, de);
            break;
    }
    return;
}

...
static int write_cmd_buffer(struct rpivid_dev *const dev,
			    ...
			    const struct rpivid_dec_state *const s)
{
	const size_t cmd_size = ALIGN(de->cmd_len * sizeof(de->cmd_fifo[0]), dev->cache_align);
	de->cmd_addr = dma_map_single(dev->dev, de->cmd_fifo, cmd_size, DMA_TO_DEVICE);
	if (dma_mapping_error(dev->dev, de->cmd_addr)) {
		v4l2_err(&dev->v4l2_dev,
			 "Map cmd buffer (%zu): FAILED\n", cmd_size);
		return -ENOMEM;
	}
	BUG_ON((de->cmd_addr & 63) != 0);

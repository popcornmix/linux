 ...
 */
#define RPIVID_H265_NEIGHBOR_INFO_BUF_SIZE	(397 * SZ_1K)
#define RPIVID_H265_ENTRY_POINTS_BUF_SIZE	(4 * SZ_1K)
#define RPIVID_H265_MV_COL_BUF_UNIT_CTB_SIZE	160

struct rpivid_h265_sram_frame_info {
...
struct rpivid_h265_sram_pred_weight {
	__s8	delta_weight;
	__s8	offset;
} __packed;

static enum rpivid_irq_status rpivid_h265_irq_status(struct rpivid_ctx *ctx)
{
#if 0
	struct rpivid_dev *dev = ctx->dev;
	u32 reg;

	reg = rpivid_read(dev, VE_DEC_H265_STATUS);
	reg &= VE_DEC_H265_STATUS_CHECK_MASK;

	if (reg & VE_DEC_H265_STATUS_CHECK_ERROR ||
	    !(reg & VE_DEC_H265_STATUS_SUCCESS))
		return RPIVID_IRQ_ERROR;
#endif
	return RPIVID_IRQ_OK;
}

static void rpivid_h265_irq_clear(struct rpivid_ctx *ctx)
{
#if 0
	struct rpivid_dev *dev = ctx->dev;

	rpivid_write(dev, VE_DEC_H265_STATUS, VE_DEC_H265_STATUS_CHECK_MASK);
#endif
}

static void rpivid_h265_irq_disable(struct rpivid_ctx *ctx)
{
	struct rpivid_dev *dev = ctx->dev;
#if 0
	u32 reg = rpivid_read(dev, VE_DEC_H265_CTRL);

	reg &= ~VE_DEC_H265_CTRL_IRQ_MASK;

	rpivid_write(dev, VE_DEC_H265_CTRL, reg);
#endif
}
#if 0
static void rpivid_h265_sram_write_offset(struct rpivid_dev *dev, u32 offset)
{
	rpivid_write(dev, VE_DEC_H265_SRAM_OFFSET, offset);
}

static void rpivid_h265_sram_write_data(struct rpivid_dev *dev, void *data,
					unsigned int size)
{
	u32 *word = data;

	while (size >= sizeof(u32)) {
		rpivid_write(dev, VE_DEC_H265_SRAM_DATA, *word++);
		size -= sizeof(u32);
	}
}

static inline dma_addr_t
rpivid_h265_frame_info_mv_col_buf_addr(struct rpivid_ctx *ctx,
...
static void rpivid_h265_frame_info_write_single(struct rpivid_ctx *ctx,
						unsigned int index,
						bool field_pic,
						u32 pic_order_cnt[],
						int buffer_index)
{
	struct rpivid_dev *dev = ctx->dev;
	dma_addr_t dst_luma_addr = rpivid_dst_buf_addr(ctx, buffer_index, 0);
	dma_addr_t dst_chroma_addr = rpivid_dst_buf_addr(ctx, buffer_index, 1);
	dma_addr_t mv_col_buf_addr[2] = {
		rpivid_h265_frame_info_mv_col_buf_addr(ctx, buffer_index, 0),
		rpivid_h265_frame_info_mv_col_buf_addr(ctx, buffer_index,
						       field_pic ? 1 : 0)
	};
	u32 offset = VE_DEC_H265_SRAM_OFFSET_FRAME_INFO +
		     VE_DEC_H265_SRAM_OFFSET_FRAME_INFO_UNIT * index;
	struct rpivid_h265_sram_frame_info frame_info = {
	...
	rpivid_h265_sram_write_offset(dev, offset);
	rpivid_h265_sram_write_data(dev, &frame_info, sizeof(frame_info));
}

static void rpivid_h265_frame_info_write_dpb(struct rpivid_ctx *ctx,
					     ...
					     u8 num_active_dpb_entries)
{
	...
	for (i = 0; i < num_active_dpb_entries; i++) {
		...
		rpivid_h265_frame_info_write_single(ctx, i, dpb[i].field_pic,
						    pic_order_cnt,
						    buffer_index);
	}
}

static void rpivid_h265_ref_pic_list_write(struct rpivid_dev *dev,
					   const struct v4l2_hevc_dpb_entry *dpb,
					   const u8 list[],
					   ...
					   u32 sram_offset)
{
	...
	rpivid_h265_sram_write_offset(dev, sram_offset);
	...
	for (i = 0; i < num_ref_idx_active; i++) {
		...
		if ((i % 4) == 3 || i == (num_ref_idx_active - 1)) {
			rpivid_h265_sram_write_data(dev, &word, sizeof(word));
			word = 0;
		}
	}
}

static void rpivid_h265_pred_weight_write(struct rpivid_dev *dev,
...
{
	struct rpivid_h265_sram_pred_weight pred_weight[2] = { { 0 } };
	unsigned int i, j;

	rpivid_h265_sram_write_offset(dev, sram_luma_offset);
	...
	for (i = 0; i < num_ref_idx_active; i++) {
		...
		if (index == 1 || i == (num_ref_idx_active - 1))
			rpivid_h265_sram_write_data(dev, (u32 *)&pred_weight,
						    sizeof(pred_weight));
	}

	rpivid_h265_sram_write_offset(dev, sram_chroma_offset);
	...
	for (i = 0; i < num_ref_idx_active; i++) {
		...
		rpivid_h265_sram_write_data(dev, &pred_weight,
					    sizeof(pred_weight));
	}
}
#endif
			      ...
			      struct rpivid_run *run)
{
	...
	if (!ctx->codec.h265.mv_col_buf_size) {
		...
		ctx->codec.h265.mv_col_buf_unit_size =
			...
			RPIVID_H265_MV_COL_BUF_UNIT_CTB_SIZE + SZ_1K;
	...
	rpivid_engine_enable(ctx, RPIVID_CODEC_H265);

	/* Source offset and length in bits. */

	reg = slice_params->data_bit_offset;
	rpivid_write(dev, VE_DEC_H265_BITS_OFFSET, reg);

	reg = slice_params->bit_size - slice_params->data_bit_offset;
	rpivid_write(dev, VE_DEC_H265_BITS_LEN, reg);
	...
	rpivid_write(dev, VE_DEC_H265_BITS_ADDR, reg);

	src_buf_end_addr = src_buf_addr +
			   DIV_ROUND_UP(slice_params->bit_size, 8);

	reg = VE_DEC_H265_BITS_END_ADDR_BASE(src_buf_end_addr);
	rpivid_write(dev, VE_DEC_H265_BITS_END_ADDR, reg);

	/* Coding tree block address: start at the beginning. */
	reg = VE_DEC_H265_DEC_CTB_ADDR_X(0) | VE_DEC_H265_DEC_CTB_ADDR_Y(0);
	rpivid_write(dev, VE_DEC_H265_DEC_CTB_ADDR, reg);

	rpivid_write(dev, VE_DEC_H265_TILE_START_CTB, 0);
	rpivid_write(dev, VE_DEC_H265_TILE_END_CTB, 0);

	/* Clear the number of correctly-decoded coding tree blocks. */
	rpivid_write(dev, VE_DEC_H265_DEC_CTB_NUM, 0);

	/* Initialize bitstream access. */
	rpivid_write(dev, VE_DEC_H265_TRIGGER, VE_DEC_H265_TRIGGER_INIT_SWDEC);
	...
	rpivid_write(dev, VE_DEC_H265_DEC_NAL_HDR, reg);
	...
	rpivid_write(dev, VE_DEC_H265_DEC_SPS_HDR, reg);
	...
	rpivid_write(dev, VE_DEC_H265_DEC_PCM_CTRL, reg);
	...
	rpivid_write(dev, VE_DEC_H265_DEC_PPS_CTRL0, reg);
	...
	rpivid_write(dev, VE_DEC_H265_DEC_PPS_CTRL1, reg);
	...
	rpivid_write(dev, VE_DEC_H265_DEC_SLICE_HDR_INFO0, reg);
	...
	rpivid_write(dev, VE_DEC_H265_DEC_SLICE_HDR_INFO1, reg);
	...
	rpivid_write(dev, VE_DEC_H265_DEC_SLICE_HDR_INFO2, reg);
	...
	rpivid_write(dev, VE_DEC_H265_DEC_PIC_SIZE, reg);

	/* Scaling list. */

	reg = VE_DEC_H265_SCALING_LIST_CTRL0_DEFAULT;
	rpivid_write(dev, VE_DEC_H265_SCALING_LIST_CTRL0, reg);

	/* Neightbor information address. */
	reg = VE_DEC_H265_NEIGHBOR_INFO_ADDR_BASE(ctx->codec.h265.neighbor_info_buf_addr);
	rpivid_write(dev, VE_DEC_H265_NEIGHBOR_INFO_ADDR, reg);

	/* Write decoded picture buffer in pic list. */
	rpivid_h265_frame_info_write_dpb(ctx, slice_params->dpb,
	...
	rpivid_h265_frame_info_write_single(ctx, output_pic_list_index,
					    slice_params->pic_struct != 0,
					    pic_order_cnt,
					    run->dst->vb2_buf.index);

	rpivid_write(dev, VE_DEC_H265_OUTPUT_FRAME_IDX, output_pic_list_index);

	/* Reference picture list 0 (for P/B frames). */
	if (slice_params->slice_type != V4L2_HEVC_SLICE_TYPE_I) {
		rpivid_h265_ref_pic_list_write(dev, slice_params->dpb,
		...
		if ((pps->flags & V4L2_HEVC_PPS_FLAG_WEIGHTED_PRED) ||
			...
			rpivid_h265_pred_weight_write(dev,
	...
	rpivid_write(dev, VE_DEC_H265_CTRL, VE_DEC_H265_CTRL_IRQ_MASK);
#endif
}
...
static int rpivid_h265_start(struct rpivid_ctx *ctx)
{
	...
	ctx->codec.h265.neighbor_info_buf =
		dma_alloc_coherent(dev->dev, RPIVID_H265_NEIGHBOR_INFO_BUF_SIZE,
...
static void rpivid_h265_stop(struct rpivid_ctx *ctx)
{
	...
	dma_free_coherent(dev->dev, RPIVID_H265_NEIGHBOR_INFO_BUF_SIZE,
			  ctx->codec.h265.neighbor_info_buf,
			  ctx->codec.h265.neighbor_info_buf_addr);
}
...
// ####### FIXME
//	rpivid_write(dev, VE_DEC_H265_TRIGGER, VE_DEC_H265_TRIGGER_DEC_SLICE);
}

struct rpivid_dec_ops rpivid_dec_ops_h265 = {
	.irq_clear	= rpivid_h265_irq_clear,
	.irq_disable	= rpivid_h265_irq_disable,
	.irq_status	= rpivid_h265_irq_status,
